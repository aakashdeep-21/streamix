...
class StreamixApiIntegrationTest {
	...
	@Test
	@Order(27)
	void longPollReturnsEarlyWhenMessageArrives() throws Exception {
		mvc.perform(post("/api/v1/topics").contentType(MediaType.APPLICATION_JSON)
						.content("{\"name\":\"lp\",\"partitions\":1}"))
				.andExpect(status().isCreated());
		mvc.perform(post("/api/v1/groups/lp-g/consumers").contentType(MediaType.APPLICATION_JSON)
						.content("{\"consumerId\":\"lp-c\",\"topics\":[\"lp\"]}"))
				.andExpect(status().isCreated());

		Thread publisher = new Thread(() -> {
			try {
				Thread.sleep(400);
				mvc.perform(post("/api/v1/topics/lp/messages").contentType(MediaType.APPLICATION_JSON)
						.content("{\"value\":\"wake-up\"}"));
			} catch (Exception ignored) {
				// failure surfaces as the poll timing out below
			}
		});
		publisher.start();
		long start = System.currentTimeMillis();
		mvc.perform(get("/api/v1/groups/lp-g/consumers/lp-c/messages").param("waitMs", "8000"))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.count").value(1));
		long elapsed = System.currentTimeMillis() - start;
		publisher.join();
		assertThat(elapsed).isBetween(200L, 7000L); // woke on publish, well before the 8s deadline
	}

	@Test
	@Order(28)
	void longPollTimesOutEmptyAndRejectsNegativeWait() throws Exception {
		long start = System.currentTimeMillis();
		mvc.perform(get("/api/v1/groups/lp-g/consumers/lp-c/messages").param("waitMs", "400"))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.count").value(0));
		assertThat(System.currentTimeMillis() - start).isGreaterThanOrEqualTo(350L);

		mvc.perform(get("/api/v1/groups/lp-g/consumers/lp-c/messages").param("waitMs", "-1"))
				.andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.error").value("INVALID_ARGUMENT"));
	}
	...
	void openApiSpecIsServed() throws Exception {
		mvc.perform(get("/v3/api-docs"))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.info.title").value("Streamix Broker API"))
				.andExpect(jsonPath("$.paths['/api/v1/topics']").exists())
				.andExpect(jsonPath("$.paths['/api/v1/topics/{topic}/messages']").exists())
				.andExpect(jsonPath("$.paths['/api/v1/groups/{group}/consumers/{consumerId}/messages']").exists());
	}
