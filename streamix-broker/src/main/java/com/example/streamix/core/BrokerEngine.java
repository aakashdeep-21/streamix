...
public class BrokerEngine {
	...
	private final Object pollSignal = new Object(); // wakes long-polling consumers on publish
	...
	// waitMs > 0 long-polls: held in 500ms slices so each pass still heartbeats the session.
	public List<PolledMessage> poll(String groupId, String consumerId, Integer requestedMax, Long waitMs) {
		...
		while (true) {
			...
			synchronized (pollSignal) {
				try {
					pollSignal.wait(Math.min(500, remaining));
